			...
			if( $EM_Ticket->get_available_spaces() > 1 && ( empty($EM_Ticket->ticket_max) || $EM_Ticket->ticket_max > 1 ) ): //more than one space available ?>				
				<div class="flex justify-between">
					...
					<?php 
						...
						$spaces_options = '<button :class="{\'bg-lightgray-lighter border-lightgray-lighter\': ticketCount' . $EM_Ticket->ticket_id . ' <= ' . $min . '}" class="rounded-tl-sm font-bold border-2 border-lightgray text-large py-1 px-4 bg-lightgray" x-bind:disabled="ticketCount' . $EM_Ticket->ticket_id . ' <= ' . $min . '" @click.prevent="ticketCount' . $EM_Ticket->ticket_id . '--">-</button>';
						$spaces_options .= '<input class="w-8 p-1 border-2 border-lightgray-lighter bg-lightgray-lighter text-center" x-on:load="ticketCount' . $EM_Ticket->ticket_id . ' = ' . $min . '" type="text" min="' . $min . '" max="' . $max . '" name="em_tickets[' . $EM_Ticket->ticket_id . '][spaces]" class="em-ticket-select" x-model="ticketCount' . $EM_Ticket->ticket_id . '" id="em-ticket-spaces-' . $EM_Ticket->ticket_id . '">';
						$spaces_options .= '<button class="rounded-br-sm font-bold border-2 border-lightgray text-large py-1 px-4 bg-lightgray" x-bind:disabled="ticketCount' . $EM_Ticket->ticket_id . ' >= ' . $max . '" @click.prevent="ticketCount' . $EM_Ticket->ticket_id . '++">+</button>';
