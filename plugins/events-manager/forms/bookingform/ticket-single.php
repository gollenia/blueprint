		...
		case 'spaces':
			if( $EM_Ticket->get_available_spaces() > 1 && ( empty($EM_Ticket->ticket_max) || $EM_Ticket->ticket_max > 1 ) ): //more than one space available ?>				
				<div class="flex justify-between">
					...
					<div class="number-picker em-bookings-ticket-table-spaces">
					<?php 
						$min=0;
						...
						$spaces_options = '<button :class="{\'bg-primary border-gray-200\': ticketCount' . $EM_Ticket->ticket_id . ' <= ' . $min . '}" class="" x-bind:disabled="ticketCount' . $EM_Ticket->ticket_id . ' <= ' . $min . '" @click.prevent="ticketCount' . $EM_Ticket->ticket_id . '--"></button>';
						$spaces_options .= '<input class="" x-on:load="ticketCount' . $EM_Ticket->ticket_id . ' = ' . $min . '" type="text" min="' . $min . '" max="' . $max . '" name="em_tickets[' . $EM_Ticket->ticket_id . '][spaces]" class="em-ticket-select" x-model="ticketCount' . $EM_Ticket->ticket_id . '" id="em-ticket-spaces-' . $EM_Ticket->ticket_id . '">';
						$spaces_options .= '<button class="" x-bind:disabled="ticketCount' . $EM_Ticket->ticket_id . ' >= ' . $max . '" @click.prevent="ticketCount' . $EM_Ticket->ticket_id . '++"></button>';
